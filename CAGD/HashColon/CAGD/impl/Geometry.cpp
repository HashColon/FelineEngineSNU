...
{
	std::array<std::vector<size_t>, 3>  TriSurfaceIndexHelper::BIdx(size_t order)
	{
		using namespace std;
	...
	std::vector<std::vector<size_t>>  TriSurfaceIndexHelper::VIdx(size_t order)
	{
		using namespace std;
	...
	std::vector<std::vector<size_t>>  TriSurfaceIndexHelper::WIdx(size_t order)
	{
		using namespace std;
		// if no matching idx info found, build one
	...
	std::vector<std::vector<size_t>>  TriSurfaceIndexHelper::UIdx(size_t order)
	{
		using namespace std;
		// if no matching idx info found, build one
	...
	std::array<std::vector<size_t>, 4> SurfaceIndexHelper::BIdx(std::array<size_t, 2> uvorders)
	{
		// if no matching bidx info found, build one
		if (_bidxlist.find(uvorders) == _bidxlist.end())
		{
			...
			std::array<std::vector<size_t>, 4> bIdx;
	...
	std::vector<std::vector<size_t>> SurfaceIndexHelper::UIdx(std::array<size_t, 2> uvorders)
	{
		// if no matching bidx info found, build one
		if (_uidxlist.find(uvorders) == _uidxlist.end())
		{
			...
			std::vector<std::vector<size_t>> uidx;
			uidx.resize(vOrder);
	...
	std::vector<std::vector<size_t>> SurfaceIndexHelper::VIdx(std::array<size_t, 2> uvorders)
	{
		// if no matching bidx info found, build one
