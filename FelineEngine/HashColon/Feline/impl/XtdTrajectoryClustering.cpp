...
namespace HashColon::Feline::XtdTrajectoryClustering
{
	...
	namespace _hidden
	{
		static shared_ptr<vector<vector<Real>>> zBvn_js;
		static shared_ptr<vector<vector<Real>>> zBvn_ws;		
		...
		vector<SampleType> GetBVNSamples(
			...
			shared_ptr<vector<vector<Real>>> zBvn,	// precomputed pdf
		...
		_WassersteinOption options)
	{
		...
		auto SamplesA = GetBVNSamples(a, aDir, options.domainUnit, options.domainSize, zBvn_ws);
		auto SamplesB = GetBVNSamples(b, bDir, options.domainUnit, options.domainSize, zBvn_ws);
		...
		_JSDivergenceOption options)
	{
		...
		auto SamplesA = GetBVNSamples(a, aDir, options.domainUnit, options.domainSize, zBvn_js, aPdfTot);
		auto SamplesB = GetBVNSamples(b, bDir, options.domainUnit, options.domainSize, zBvn_js, bPdfTot);
		int k = (int)floor(options.domainSize / options.domainUnit);
		int N = (2 * k + 1) * (2 * k + 1);
...
namespace HashColon::Feline::XtdTrajectoryClustering
{
	...
	void DtwXtd_usingJSDivergence::Initialize(const string configFilePath)
	{
		...
		cli->add_option("--MonteCarloDomainUnit", _cDefault.js.domainUnit,
			"Interval of Monte Carlo integration samples in the unit of sigma.");

		cli->add_option("--MonteCarloDomainSize", _cDefault.js.domainSize,
			"Size of sampling range of  Monte Carlo integration samples in the unit of sigma.");

		cli->add_option("--MonteCarloErrorEpsilon", _cDefault.js.errorEpsilon,
		...
		for (size_t i = 0; i < a.size(); i++)
		{
			for (size_t j = 0; j < b.size(); j++)
			{
				...
				else
				{
					...
					warp[i][j] =
						JSDivergenceDistance(a[i], aDir, b[j], bDir, _c.js)
						+ min({
						(i >= 1 ? warp[i - 1][j] : numeric_limits<Real>::max()),
						(j >= 1 ? warp[i][j - 1] : numeric_limits<Real>::max()),
						((i >= 1 && j >= 1) ? warp[i - 1][j - 1] : numeric_limits<Real>::max())
	...
	void DtwXtd_usingWasserstein::Initialize(const string configFilePath)
	{
		...
		cli->add_option("--MonteCarloDomainUnit", _cDefault.ws.domainUnit,
			"Interval of Monte Carlo integration samples in the unit of sigma.");

		cli->add_option("--MonteCarloDomainSize", _cDefault.ws.domainSize,
			"Size of sampling range of  Monte Carlo integration samples in the unit of sigma.");

		cli->add_option("--MonteCarloErrorEpsilon", _cDefault.ws.errorEpsilon,
			"Error threshold for Monte Carlo integration.");
	}
		...
		for (size_t i = 0; i < a.size(); i++)
		{
			for (size_t j = 0; j < b.size(); j++)
			{
				...
				else
				{
					...
					warp[i][j] =
						WassersteinDistance(a[i], aDir, b[j], bDir, _c.ws)
						+ min({
						(i >= 1 ? warp[i - 1][j] : numeric_limits<Real>::max()),
						(j >= 1 ? warp[i][j - 1] : numeric_limits<Real>::max()),
						((i >= 1 && j >= 1) ? warp[i - 1][j - 1] : numeric_limits<Real>::max())
	...
	void DtwXtd_BlendedDistance::Initialize(const string configFilePath)
	{
		...
		cli->add_option("--JsDomainUnit", _cDefault.js.domainUnit,
			"Interval of Monte Carlo integration samples in the unit of sigma.");

		cli->add_option("--JsDomainSize", _cDefault.js.domainSize,
			"Size of sampling range of  Monte Carlo integration samples in the unit of sigma.");

		cli->add_option("--JsErrorEpsilon", _cDefault.js.errorEpsilon,
			"Error threshold for Monte Carlo integration.");

		cli->add_option("--WsDomainUnit", _cDefault.ws.domainUnit,
			"Interval of Monte Carlo integration samples in the unit of sigma.");

		cli->add_option("--WsDomainSize", _cDefault.ws.domainSize,
			"Size of sampling range of  Monte Carlo integration samples in the unit of sigma.");

		cli->add_option("--WsErrorEpsilon", _cDefault.ws.errorEpsilon,
			"Error threshold for Monte Carlo integration.");
		...
		for (size_t i = 0; i < a.size(); i++)
		{
			for (size_t j = 0; j < b.size(); j++)
			{
				...
				else
				{
					...
					//// debug
					/*logger.Log() << "flag a" << endl;
					Real D_Euclidean = a[i].Pos.DistanceTo(b[j].Pos);
					logger.Log() << "flag b" << endl;
					Real D_JS = JSDivergenceDistance(a[i], aDir, b[j], bDir, _c.js);
					logger.Log() << "flag c" << endl;
					Real D_PF = PFDistance(a[i], aDir, b[j], bDir, { _c.Pf_XtdSigmaRatio });
					logger.Log() << "flag d" << endl;
					Real D_WS = WassersteinDistance(a[i], aDir, b[j], bDir, _c.ws);
					logger.Log() << "flag e" << endl;*/

					warp[i][j] =
						(_c.Coeff_Euclidean > 0 ? _c.Coeff_Euclidean * a[i].Pos.DistanceTo(b[j].Pos) : 0 ) +
						(_c.Coeff_JS > 0 ? _c.Coeff_JS * JSDivergenceDistance(a[i], aDir, b[j], bDir, _c.js) : 0) +
						(_c.Coeff_PF > 0 ? _c.Coeff_PF * PFDistance(a[i], aDir, b[j], bDir, { _c.Pf_XtdSigmaRatio }): 0) +
						(_c.Coeff_WS > 0 ? _c.Coeff_WS * WassersteinDistance(a[i], aDir, b[j], bDir, _c.ws) : 0) +
						+ min({
						(i >= 1 ? warp[i - 1][j] : numeric_limits<Real>::max()),
						(j >= 1 ? warp[i][j - 1] : numeric_limits<Real>::max()),
						((i >= 1 && j >= 1) ? warp[i - 1][j - 1] : numeric_limits<Real>::max())
							});
					/*logger.Log() << "flag f" << endl;
					logger.Log() << "flag g" << endl;*/
					assert(!isnan(warp[i][j]));
					assert(warp[i][j] >= 0);
				}
			}
