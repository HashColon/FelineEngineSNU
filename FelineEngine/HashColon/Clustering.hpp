...
namespace HashColon::Clustering
{
	...
	Distance: increases as two data is more different
	Similarity: increases as two data is more similar
	*/ 
...
namespace HashColon::Clustering::Evaluation
{
	struct EvalOptions
	{
		// Add items to exclude from the evaluations
		std::vector<size_t> ExcludeItemIndices;		
		// Add clusters to exclude from the evaluations. Ex) noise cluster
		std::vector<size_t> ExcludeClusterIndices;		

		// Returns boolean vector which j-th value is true if the j-th item should be excluded
		std::vector<bool> GetExcludeItemFlags(const size_t itemN);
		// Returns boolean vector which j-th value is true if the j-th cluster should be excluded
		std::vector<bool> GetExcludeClusterFlags(const size_t clusterN);
	};

	HASHCOLON_NAMED_EXCEPTION_DEFINITION(Evaluation);	

	size_t GetMedian(const size_t clusterIdx, 
		const std::vector<size_t>& Labels, const Eigen::MatrixXR& DistanceMatrix, 
		EvalOptions opt = EvalOptions());
	std::vector<size_t> GetMedians(
		const std::vector<size_t>& Labels, const Eigen::MatrixXR& DistanceMatrix,
		EvalOptions opt = EvalOptions());

	HashColon::Real GetSilhouette(size_t itemIdx, const std::vector<size_t>& Labels, const Eigen::MatrixXR& DistanceMatrix);
	std::vector<HashColon::Real> GetSilhouettes(const std::vector<size_t>& Labels, const Eigen::MatrixXR& DistanceMatrix);

	/*
	* Compute Silhouette coefficient:
	* Silhouettes: Pre-computed silhouettes can be given as input. 
	*              If pointer of empty vector is given, silhouettes are computed and returned through this variable.
	*/
	HashColon::Real GetSilhouetteCoefficient(
		const std::vector<size_t>& Labels, const Eigen::MatrixXR& DistanceMatrix, 
		const std::shared_ptr<std::vector<HashColon::Real>> Silhouettes = nullptr,
		EvalOptions opt = EvalOptions());

	/*
	* Compute Davies-Bouldin index using median instead of average.q - value used for computing standard deviation is 1.
	* Medians: indices of medians for each cluster. if not given, computes internally
	* nearZero: value to prevent zero-division problem. Distance btwn two medians are at least this values.
	*/ 
	HashColon::Real DaviesBouldinIndex_usingMedian(
		const std::vector<size_t>& Labels, const Eigen::MatrixXR& DistanceMatrix, 
		const std::shared_ptr<std::vector<size_t>> Medians = nullptr, EvalOptions opt = EvalOptions(), const Real nearZero = 1e-6);
